...
class Study_catgory extends MY_Controller {
    ...
    public function catgory_add() {
        ...
        $cate_rules = array(
            array(
                'field' => 'catgory_name',
                'label' => '栏目名称',
                'rules' => 'trim|required|callback_check_catname'
            )
        );
        ...
        $data['catgory_list'] = $this->db->order_by('catgory_path')->get('study_catgory')->result_array();
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            $this->load->view('manageroot/catgory_add',$data);
        }
    }
        ...
        if (empty($single_info)) {
            ...
            $cate_rules = array(
                array(
                    'field' => 'catgory_name',
                    'label' => '分类名称',
                    'rules' => 'trim|required|callback_check_catgory_name'
                )
            );
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $data['catgory_info'] = $this->db->where(array('catgory_id'=>$single_info['parent_id']))->get('study_catgory')->row_array();
    ...
    public function check_catgory_name($catgory_name) {
        $contal = $this->db->where(array('catgory_name' => $catgory_name))->get('study_catgory')->num_rows();
        if ($contal) {
            $this->form_validation->set_message('check_catgory_name', '分类名未修改或%s已经存在');
            return FALSE;
        } else {
            return TRUE;
        }
